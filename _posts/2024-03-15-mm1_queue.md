...
정확하게는 도착률(arrival rate)이 $\lambda$이고 출발률(departure rate)가 $\mu$인 M/M/1 대기열(queue)에서 대기열의 길이 (queue length) $N$의 평균이
...
종합하면, arrival process는 확률과정의 일종인데, (1), (2), (3)의 서로 다른 세 방식의 확률과정으로 해석할 수 있습니다.

![]({{site.url}}\images\2024-03-15-mm1_queue\three_stochastic_processes.png){: .img-80-center}

### 1.1.3 renewal process

만약 arrival의 발생이 서로 독립적이고 각 arrival이 발생조건이 일정하다면 문제 상황을 조금 더 간결하게 만들 수 있습니다.
...
$$f_X(x)=\lambda e^{-\lambda x}\quad(x\ge0)\tag{4}$$
...
이때, $N_1$의 분포를 Poisson distribution이라고 합니다.
$t=1$이므로, Poisson distribution은 단위시간동안의 arrival의 횟수의 분포를 의미합니다.

1.2.1에서 평균적으로 $\frac1\lambda$만큼의 시간간격마다 arrival이 발생한다고 했었습니다.
따라서, '단위시간마다 평균적으로 $\lambda$번의 arrival이 발생'합니다.

![]({{site.url}}\images\2024-03-15-mm1_queue\unit_interval.png){: .img-80-center}
...
![]({{site.url}}\images\2024-03-15-mm1_queue\sub_interval.png){: .img-80-center}
...
$$
P(N=n)=\frac{\lambda^n}{n!}e^{-\lambda}\quad(n=0,1,2,\cdots)\tag{9}
$$

입니다.
이 분포를 Poisson distribution이라고 부릅니다.
...
&=e^{-\lambda}\sum_{n=1}^\infty \frac{\lambda^{n-1}}{(n-1)!}\times\lambda\\
&=e^{-\lambda}\sum_{n'=0}^\infty \frac{\lambda^{n'}}{n'!}\times\lambda\\
&=e^{-\lambda}\times e^\lambda\times\lambda\\
&=\lambda
\end{align*}
$$
...
그리고 여기서 말하는 arrival은 '사건'을 의미하는 것으로 '카페에 손님이 들어오는 사건', '배달 주문이 접수되는 사건', '방사성 물질에서 방사선이 방출되는 사건' 같은 것을 의미합니다.
</div>
...
손님이 더 들어오면 줄은 길어질 것이고, 앞 손님이 주문을 마치쳐서 줄에서 빠져나가면 줄은 짧아질 겁니다.
...
어떤 시점 $t$에 주차장에 주차되어 있는 차량의 대수를 $N_t$라고 할 때, $N_t$에 대한 분포를 계산해보는 문제를 생각할 수 있습니다.
...
> A *queue* or *queueing node* can be thought of as nearly a black box.
> *Jobs* (also called *customers* or *requests*, depending on the field) arrive to the queue, possibly wait some time, take some time being processed, and then depart from the queue.
...
예를 들어, 카페 안에 키오스크가 여러 대 있을 수도 있을 겁니다.
아래 그림은 세 개의 server가 존재하는 queueing system을 나타내고 있습니다.

![]({{site.url}}\images\2024-03-15-mm1_queue\queue_with_three_servers.png){: .img-80-center}
...
이것은, arrival과 departure가 각각 단위시간동안 $\lambda$회, $\mu$회 일어난다는 것을 의미합니다.

$$
\begin{align*}
\lambda &=\mathbb\left[\text{\# arrivals in unit time}\right]\\
\mu     &=\mathbb\left[\text{\# departure in unit time}\right]
\end{align*}
$$

![]({{site.url}}\images\2024-03-15-mm1_queue\mm1_queueing_node.png){: .img-40-center}

# 2.3 queue length $N_t$

queue의 상태는 실시간으로 변합니다.
예를 들어, 대기열의 길이(queue length)는 시간이 지남에 따라 바뀝니다.
시각 $t$에서의 대기열의 길이를 $N_t$라고 하면, 확률변수 $N_t$ 시간에 따른 추이를 생각해볼 수 있습니다.

![]({{site.url}}\images\2024-03-15-mm1_queue\transition_unit_interval.png){: .img-80-center}

예를 들어, 시각 $t$에서의 대기열의 길이가 $2$였다고 할 때, 시각 $t+1$에서의 대기열의 길이는 $3$으로 바뀔 수 있습니다.

확률변수 $N_t$의 확률질량함수를 $p_n(t)$로 표시하면, 즉

$$P\left(N_t=n\right)=p_n(t)$$

라고 하면,

$$p_0(t) + p_1(t) + \cdots + p_n(t) + \cdots = 1$$

이 성립합니다.

$p_n(t)$의 변화를 정확하게 분석하기 위해서는 Poisson distribution 식을 유도했을 때처럼 시간을 잘게 쪼개는 방법을 사용할 수 있습니다.
단위시간동안 arrival은 $\lambda$번 발생하므로, $\frac1m$ 만큼의 시간 동안에는 arrival이 $\frac\lambda m$번 발생합니다.
마찬가지로, $\frac1m$ 만큼의 시간동안 departure은 $\frac\mu m$번 발생합니다.

![]({{site.url}}\images\2024-03-15-mm1_queue\transition_sub_interval.png){: .img-80-center}

$n$이 충분히 크면, 구간 $\left[t, t+\frac1n\right]$동안 arrival 또는 departure이 두 번 이상 발생하지 않았다고 가정할 수 있습니다.
다시 말해, $\frac\lambda m$의 확률로 arrival이 발생하고, $\frac\mu m$의 확률로 departure이 발생합니다.
그러면, $p_n(t)$에 대한 점화식을 다음과 같이 얻을 수 있습니다 ; 

$$
p_0\left(t+\frac1m\right)&=\frac\mu mp_1(t)\\
p_1\left(t+\frac1m\right)&=\frac\lambda mp_0(t)+\left(1-\frac\lambda m\right)\left(1-\frac\mu m\right)p_1(t)+\frac\mu mp_2(t)\\
p_2\left(t+\frac1m\right)&=\frac\mu mp_1(t)
$$
