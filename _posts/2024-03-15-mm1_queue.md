...
시각 $t$에서의 대기열의 길이를 $K_t$라고 하면, 확률변수 $N_t$ 시간에 따른 추이를 생각해볼 수 있습니다.
...
$p_k(t)$의 변화를 정확하게 분석하기 위해서는 Poisson distribution 식을 유도했을 때처럼 시간을 잘게 쪼개는 방법을 사용할 수 있습니다.
...
&&(k=0)\\
p_k\left(t+\frac1m\right)
&=\frac\lambda mp_{k-1}(t)+p_k(t)+\frac\mu mp_{k+1}(t)
&&(k=1,2,3,\cdots)
\end{align*}
$$
...
그런 의미에서 다음과 같은 M/M/1 queue model은 다음과 같은 (triadiagonal) transition matrix로 표현할 수 있습니다.

![]({{site.url}}\images\2024-03-15-mm1_queue\transition_matrix.png){: .img-80-center}



즉, $p_k\left(t+\frac1m\right)$는 다음과 같이 $p_k(t)$로 
