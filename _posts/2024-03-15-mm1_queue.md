...
$$\mathbb E[N] = \frac\rho{1-\rho}\tag{1}$$
...
$$
p_k'(t)=-\lambda p_k(t)+\mu p_{k+1}(t)
$$
...
라고 하면 이 함수는 $\vert x\vert\lt1$인 조건에서
...
$$
\sum_{k=1}^\infty kx^k=\frac1{(1-x)^2}
$$
...
그밖에, 확률론적인 개념들을 정확하게 서술할 필요도 있습니다.

하지만, 당분간은 글의 내용을 더 추가하기보다는, 이미 쓴 내용들에서 잘못된 부분이 있는지만 수정하는 정도로만 그칠 것 같습니다.