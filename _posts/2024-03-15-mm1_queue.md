...
![]({{site.url}}\images\2024-03-15-mm1_queue\three_stochastic_processes.png){: .img-50-center}

![]({{site.url}}\images\2024-03-15-mm1_queue\mm1_queueing_node.png){: .img-50-center}
...
![]({{site.url}}\images\2024-03-15-mm1_queue\1-1-1.png){: .img-80-center}

<!-- 사건이 일어나는 시점 $S_i$이 일정하게 결정되어 있지 않고 확률적으로 발생한다면, 각각의 실수 $S_i$는 확률변수로 해석될 수 있습니다.
따라서 arrival process는 확률변수 $S_i$들의 집합

$$\{S_i:i=1,2,3,\cdots\}\tag{1.2}$$

으로 표현될 수 있습니다.

이때, 인접한 두 사건의 시간간격을 $X_i$라고 하면, 다시 말해
...
![]({{site.url}}\images\2024-03-15-mm1_queue\1-1-2.png){: .img-80-center}

로 표현할 수 있을 것입니다.
그러면 $X_i$들도 확률변수라고 말할 수 있으므로 arrival process는 확률변수 $X_i$들의 집합

$$\{X_i:i=1,2,3,\cdots\}\tag{1.3}$$
...
Poisson process는 확률과정의 일종으로, 어떤 사건이 시간 $t>0$에 따라 반복적으로 일어날 수 있는 상황에서 나타납니다.
만약 사건이 시각 $t=S_1$, $t=S_2$, $t=S_3$, $\cdots$에 차례대로 발생했다고 가정하면 ($S_1<S_2<S_3<\cdots$) 각각의 $S_i$들은 확률변수이므로

$$\{S_i:i=1,2,3,\cdots\}\tag{1.2}$$

는 확률과정입니다.
이러한 확률과정을 MIT 자료에서는 arrival process라고 부르고 있습니다.
...
도 확률과정입니다.
만약 $i$의 값에 관계없이 연속확률변수 $X_i$의 분포가 일정하고 독립적이면 (IID : $X_i$'s are independent and identicall distributed) 이러한 확률과정을 이해하고 계산하는 데 용이합니다.
이처럼 확률과정이 IID인 경우를 MIT 자료에서는 renewal process라고 명명하고 있습니다.

## 1.4 Poisson process

특히, $X_i$의 PDF가

$$f_{X_i}(x)=\lambda e^{-\lambda x}\quad(x\gt0)\tag{1.4}$$

로 주어지면, 이 확률과정을 Poisson process라고 부릅니다.
...
Poisson process를 바라보는 또다른 중요한 관점이 있습니다.
만약, 시각 $t>0$에서의 사건이 발생한 횟수를 $N(t)$라고 하면, $N(t)$ 또한 확률변수입니다.
또한, 이 이산확률변수들의 집합인
...
다시말해, Poisson process란 식 (1.4)를 만족시키는 확률과정 (1.2) 또는 확률과정 (1.3) 또는 확률과정 (1.5)를 말합니다.
같은 현상을 이야기함에도 불구하고 세가지 방식의 확률변수로 해석할 수 있는 것입니다.
그 중 $X_i$와 $N(t)$를 주로 사용하게 됩니다.
아래 그림은 $S_i$, $X_i$, $N(t)$ 사이의 관계를 표현하고 있습니다.

![]({{site.url}}\images\2024-03-17-mm1_queue\three_stochastic_processes.png){: .img-50-center}
...
$$f_X(x)=\lambda e^{-\lambda x}\quad(x\gt0)$$
...
이것은 아까 $X_i$가 IID하다고 할 때 independent 조건에 해당하는 것으로, 이전에 사건이 발생했는지의 여부가, 앞으로의 사건 발생 빈도에 영향을 미치지 않음을 의미합니다.

정확하게는, 임의의 두 양수 $x_1$, $x_2$에 대하여 다음이 성립하면 memoryless property가 성립한다고 말합니다;
...
식 (1.7)은 조건부확률의 정의에 의해 다음 식과 동치입니다 ;

$$P(X\gt x_1+x_2)=P(X\gt x_1)P(X\gt x_2)$$

그리고 이 식은 (1.6)에 의해 쉽게 증명됩니다;

$$
\begin{align*}
P(X\gt x_1)P(X\gt x_2)
&=\left(1-F_X(x_1)\right)\left(1-F_X(x_2)\right)\\
&\stackrel{(1.6)}=e^{-\lambda x_1}e^{-\lambda x_2}\\
&=e^{-\lambda(x_1+x_2)}\\
&\stackrel{(1.6)}=1-F_X(x_1+x_2)\\
&=P(X\gt x_1+x_2)
\end{align*}
$$

흥미로운 것은, memoryless property를 가지는 유일한 연속확률분포가 지수분포라는 것입니다.
증명을 위해 연속확률변수 $X$가 memoryless property (1.7)을 가진다고 가정하고 $h(x)=\ln P(X\gt x)$로 두면,
...
$$
h(qx)=h\left(\frac nmx\right)=h\left(n\cdot\frac1mx\right)=nh\left(\frac 1mx\right)=\frac nmh(x) = qh(x)
$$
...
(1.8)과 (1.9)에 의해, $h(x)$는 선형함수이고, 따라서

$$h(x)=\ln P(X\gt x)=-\lambda x$$

인 $\lambda\gt0$가 존재합니다.
따라서
