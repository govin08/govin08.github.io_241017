...
학부에 다니면서는 확률 공부를 아예 하지 않았고, 대학원에 있으면서도 확률을 다루기는 했지만, 큐잉이론은 공부할 일이 없었습니다.
...
이렇듯, 많은 현상들이 Poisson process으로 해석될 수 있습니다.
정확하게는, 어떤 사건이 시간에 따라 반복적으로 일어날 수 있다고 할 때, 사건이 일어날 것이라고 기대되는 정도가 이전의 사건 발생에 관계없다고 가정될 때, Poisson process를 생각할 수 있습니다.

## 1.1 stochastic process

확률변수들의 집합(a collection of random variables)을 확률과정(stochastic process)라고 합니다.
즉, 공집합이 아닌 집합 $J$(index set)에 대하여
...
Poisson process는 확률과정의 일종으로, 어떤 사건이 시간 $t>0$에 따라 반복적으로 일어날 수 있는 상황에서 발생합니다.
...
이러한 확률과정을 MIT 자료에서는 renewal process라고 명명하고 있습니다.
...
그러면 위 식은 $x_2$의 시간동안 사건이 발생하지 않았다고 가정할 때, 그다음 $x_1$만큼의 시간동안 사건이 발생하지 않을 확률이, 현재시점으로부터 $x_2$의 시간동안 사건이 발생하지 않을 확률과 같다는 것을 뜻합니다.
...
증명을 위해 연속확률변수 $X$가 memoryless property (7)을 가진다고 가정하고 $h(x)=\ln P(X\gt x)$로 두면,
...
입니다.
마지막으로 극한의 개념을 적절히 사용하면 실수 $c$에 대하여
