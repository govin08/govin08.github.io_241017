...
또한, 중고등학교 수학에서도 확률을 공부하는데 거기에서 배우는 확률의 개념은 꽤 직관적이고 기초적인 확률입니다.
...
다만, 이와 같은 공리론적 접근을 소개하는 것은 논의를 완전하게 하고 싶어서이고 개인적으로 쓰고 싶어서이지 뒤이어 나올 내용들과 많이 연관되지는 않습니다.
...
<!-- 고등학교의 정의로 넘어가기 전에, 몇 개 용어들은 짚고 넘어가면 좋을 것 같습니다.
...
우리는 모든 가능한 경우들의 집합을 '표본공간'이라는 말로 표현했고, '현재 관심있는 경우들의 집합'을 '사건'이라는 말로 표현한 것입니다.

몇 가지 기본적인 용어들을 더 소개하겠습니다.
공집합($\varnothing$)과 전체집합($S$) 그 자체로 $S$의 부분집합이므로 각각 사건이라고 말할 수 있습니다.
$\varnothing$은 공사건, $S$는 전사건이라고 불립니다.
또한, $A$의 여집합인 $A^c$도 $S$의 부분집합이므로 사건이라고 말할 수 있을텐데 이것은 $A$의 여사건이라고 불립니다.

한편, 두 사건 $A$, $B$가 서로소이면 ($A\cap B=\varnothing$)이면, 두 사건이 서로 배반사건(exclusive events)이라고 말합니다. 
...
대부분의 학생들은 확률의 정의가 정확히 무엇인지도 모른 채로 넘어가곤 합니다.
...
이때, $S$는 공집합이 아닌 집합이고, $\Sigma$는 특정한 조건을 만족시키는 집합이며, $P$도 특정한 조건을 만족시키는 함수입니다.
...
$P$는 정의역이 $\Sigma$이고 공역이 $[0,1]$인 함수($P:\Sigma\to[0,1]$)로서 다음 두 조건을 만족시키는 함수입니다.
...
이와 같은 $P$는 확률측도(probability measure)라고도 불리며, 실제로 주어진 사건에 대한 확률을 계산하는 함수입니다.
...
2. 만약, $A$와 $B$가 배반(exclusive) 사건이면, $P(A\cup B)=P(A)+P(B)$가 성립합니다.
왜냐하면
...
그리고 이렇게 정의한 확률의 개념은 고등학교 과정의 확률의 개념을 아우르는 개념입니다.
...
위의 설명에서, 서로 배반인 사건들의 합집합에 대한 확률이 각각의 사건들의 확률의 합과 같다는 것을 언급했고 또 증명했습니다.
이것은 <b> 합의 법칙 </b> 이라고 불리는 법칙입니다.
...
지금까지 기초적인 확률의 정의와 더불어 공리적인(측도론적인) 정의도 살펴봤습니다.
굳이 이렇게까지 어렵게 확률을 정의할 필요가 있느냐? 라는 질문에는 이미 $S$가 무한집합인 경우에 확률의 정의가 애매해진다는 것을 언급한 바 있습니다.
조금 앞으로의 내용과 관련되어 있는 이유는, 기존의 방법으로는 연속확률분포를 설명하기가 어렵다는 점입니다.
이 문제는 나중에 연속확률분포에 대해 이야기하면서 다시 다루게 될 것 같습니다.

<b>참고한 책들</b>
- 김원경 외, 「중학교 수학 2」, 비상출판사
- 김원경 외, 「확률과 통계」, 비상출판사